...
falls inside tomorrow night's LSST footprint.
...
def resolve_night(date_str: str | None) -> tuple[date, float]:
    """Resolve the --date argument to a (calendar date, start MJD) pair."""
    night_date = date.fromisoformat(date_str) if date_str else default_night_date()
    if night_date < default_night_date():
        raise ValueError(f"requested night {night_date} is in the past")
    return night_date, night_start_mjd(night_date)


def fetch_lsst_pointings(start_mjd: float, window_hours: float) -> pd.DataFrame:
    """Fetch the scheduled LSST pointings for the given night from obsloctap."""
...
def is_public_gwtc_event(event: GWEvent) -> bool:
    ...
    against GWOSC's public event list instead.
    """
    ...
    if gps > O4B_END_GPS:
        return False
    return bool(gwosc_datasets.find_datasets(type="event", segment=(gps - 1, gps + 1)))
...
def fetch_candidate_events(
    ...
    By default queries GraceDB live. In offline mode, events are instead loaded
    ...
    since offline mode does not hit the network to download missing ones.
    ...
    `is_public_gwtc_event`), excluding non-public/unvetted superevents.
    """
    if offline:
    ...
    if public_only:
        events = [ev for ev in events if is_public_gwtc_event(ev)]
    return events
...
def plot_night_overview(footprint, ranking: pd.DataFrame, events_by_id: dict, output_dir: Path) -> Path:
    ...
    def to_mollweide(ra_deg, dec_deg):
        return np.radians(ra_deg - 180), np.radians(dec_deg)
    ...
    def ra_hours_formatter(x, pos):
        ra_deg = (np.degrees(x) + 180.0) % 360.0
    ...
    ax.set_title(f"GW events in LSST footprint (n={len(ranking)})", color="white", fontsize=14, pad=15)
    ax.legend(
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="ISO date (YYYY-MM-DD) of the observing night to analyze. Defaults to the "
             "current or next LSST night, depending on Chile local time.",
    )
    parser.add_argument(
        "--window-hours", type=float, default=24.0,
        help="Duration in hours of the LSST schedule window fetched from the night start.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Restrict candidates to events already released in the public GWTC "
             "catalog through O4b, excluding non-public/unvetted superevents.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Do not cache the selected GWEvent records to disk.",
    )
...
def main(argv: list[str] | None = None) -> list[str]:
    ...
    try:
        night_date, start_mjd = resolve_night(args.date)
    except ValueError as e:
        sys.exit(f"error: {e}")

    LOGGER.info("Selecting events for the night of %s (MJD %.2f)", night_date, start_mjd)
    configure_skymap_directory(args.skymap_dir)

    try:
        pointings = fetch_lsst_pointings(start_mjd, args.window_hours)
    ...
    candidates = fetch_candidate_events(
        args.far_threshold, args.terrestrial_threshold, offline=args.offline,
    ...
    if not args.no_plots:
        fig_path = plot_night_overview(footprint, ranking, events_by_id, args.output_dir)
